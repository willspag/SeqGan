...
'''
1) Added BatchNorm for faster Training
2) Made LSTM Bidirectional
3) Used Leaky-Relu instead of normal relu
4) Added num_layers option
5) Added Dropout option
6) Added L2 kernel regulization option
'''
...
class Generator(object):
    def __init__(self, train_data_loader, validation_data_loader, units = 256, leaky_relu_alpha = 0.1, 
                ...
                sequence_length = 128, loss = 'categorical_crossentropy',metrics = ['loss','val_loss']):
    ...
    def pretrain(self, num_epochs, verbose = 1, callbacks=[], save_weights = False, filepath = ''):
    ...
    def generate(self, gen_batch_size):
        smiles = ['G' for _ in range(gen_batch_size)]
        gen_samples = np.array(self.st.one_hot_encode(smiles))
        gen_samples = np.reshape(gen_samples,(gen_batch_size,1))

        for i in range(self.seq_len-1):
            preds = self.model(gen_samples)
            preds = tf.argmax(preds, axis = 1)
            gen_samples = np.hstack(gen_samples, preds)
